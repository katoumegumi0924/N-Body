...
public class AstroPhysics
{
    ...
    private AstroProto debrisProto;

    // 暂存爆炸请求
    private List<ExplosionRequest> explosionRequestList = new List<ExplosionRequest>();

    public AstroPhysics(AstroData data, GameConfig config, AstroProto debrisProto)
    {
        _data = data;
        _config = config;
        this.debrisProto = debrisProto;
    }
    ...
    private void HandleCollision()
    {
        ...
        float immunityDuration = _config.spawnImmunityTime;
    ...
    private void ProcessCollision(ref Astro major, ref Astro minor)
    {
        ...
        // 质量悬殊，吞噬
        if (massRatio > _config.swallowThreshold)
        {
            MergeAstro(ref major, ref minor);
        }
        // 两个大质量天体，融合 分裂
        else if (major.mass > _config.hugeMass && minor.mass > _config.hugeMass)
        {
    ...
    private void MergeAndExplode(ref Astro major, ref Astro minor)
    {
        float totalMass = major.mass + minor.mass;
        // 根据合并损耗率计算碎片质量与合并后质量
        float debrisTotalMass = totalMass * _config.lossRatio;
    ...
    public void ProcessExplosion(AstroProto debrisProto)
    {
        for (int i = 0; i < explosionRequestList.Count; ++i)
        {
            ...
            for (int j = 0; j < explosionRequestList[i].count; ++j)
            {
                ...
                Vector2 spawnVel = explosionRequestList[i].velocity + dir * Random.Range(_config.minDebrisSpeed, _config.maxDebrisSpeed);
